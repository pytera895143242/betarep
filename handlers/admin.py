...
async def posting_o(chat_ids,second_start,list_otvetok,second_work):
    ...
    for link in list_otvetok:
        del_id = []
        for i in chat_ids:
            # print('Выкладываю в ', i)
            try:
                m = await bot.send_message(chat_id = i, text=(array_post[random.randint(0, 7)]).format(link))
                del_id.append(m.message_id)
            except:
                pass

        # print(life)
        # print('ID Для удаления:', len(del_id), del_id)
        # print(chat_ids)
        await asyncio.sleep(life-1)

        for iq in range(0,len(del_id)):
            # print('Удаляю в ', chat_ids[iq] )
            try:
...
async def dogon1(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(chat_id=message.chat.id,text="""На какие ссылки делаем догон? Каждая ссылка с новой строки""")
...
async def dogon2(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(chat_id=message.chat.id,text="""Введите время старта догона по МСК""")
...
async def dogon3(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(chat_id=message.chat.id,text="""Введите время окончания догона по МСК""")
